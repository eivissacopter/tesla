"""
utils.py - Data fetching and cleaning utilities for the Tesla Battery Analysis Dashboard
"""
...
def clean_numeric_col(series, remove_unit=None):
    """
    Clean a numeric column by removing units, replacing commas with dots, 
    ...
    Returns:
        Cleaned pandas Series as numeric
    """
    # Make a copy to avoid modifying the original
    cleaned = series.copy()
    
    # Remove the specific unit if provided
    if remove_unit:
        cleaned = cleaned.str.replace(remove_unit, '', regex=False)
...
def fetch_data(username_filter=None):
    ...
    # Clean up the 'Odometer' column to ensure it is numeric - Fixed regex with raw string
    ...
    # Clean 'Rated Range' using the helper function
    df['Rated Range'] = clean_numeric_col(df['Rated Range'], remove_unit=' km')

    # Clean 'Capacity Net Now' using the helper function
    df['Capacity Net Now'] = clean_numeric_col(df['Capacity Net Now'], remove_unit=' kWh')

    # Convert Degradation to numeric
    df['Degradation'] = clean_numeric_col(df['Degradation'].str.replace('%', '', regex=False))

    # Clean 'Daily SOC Limit' using the helper function
    df['Daily SOC Limit'] = clean_numeric_col(df['Daily SOC Limit'].str.replace('%', '', regex=False))
    
    # Clean 'DC Ratio' using the helper function
    df['DC Ratio'] = clean_numeric_col(df['DC Ratio'].str.replace('%', '', regex=False))
...
def fetch_battery_info():
    ...
    # Fixed deprecation: Replace applymap with map
