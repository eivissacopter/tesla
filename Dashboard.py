...
# Iterate over each battery type in the filtered dataframe
for battery_type in st.session_state.filtered_df['Battery'].unique():
    battery_df = st.session_state.filtered_df[st.session_state.filtered_df['Battery'] == battery_type]

    if not battery_df.empty:
        # Polynomial regression to fit the degradation curve for Age
        X_age = battery_df['Age'].values.reshape(-1, 1)
        y_degradation = battery_df['Degradation'].values

        poly_features_age = PolynomialFeatures(degree=3)
        X_poly_age = poly_features_age.fit_transform(X_age)
        poly_reg_model_age = LinearRegression()
        poly_reg_model_age.fit(X_poly_age, y_degradation)

        # Function to find the root (age at -30% degradation)
        def age_degradation_eq(age):
            return poly_reg_model_age.predict(poly_features_age.transform([[age]]))[0] + 30

        projected_age_months = fsolve(age_degradation_eq, x0=100)[0]
        projected_age_years = projected_age_months / 12

        # Polynomial regression to fit the degradation curve for Odometer
        X_odo = battery_df['Odometer'].values.reshape(-1, 1)

        poly_features_odo = PolynomialFeatures(degree=3)
        X_poly_odo = poly_features_odo.fit_transform(X_odo)
        poly_reg_model_odo = LinearRegression()
        poly_reg_model_odo.fit(X_poly_odo, y_degradation)

        # Function to find the root (odometer at -30% degradation)
        def odo_degradation_eq(odo):
            return poly_reg_model_odo.predict(poly_features_odo.transform([[odo]]))[0] + 30

        projected_odo_km = fsolve(odo_degradation_eq, x0=200000)[0]

        # Append the results to the list
        projection_results.append({
            'Battery': battery_type,
            'Projected Age (years)': round(projected_age_years, 2),
            'Projected Odometer (km)': round(projected_odo_km, 2)
        })

# Convert the results to a DataFrame and display as a table
projection_df = pd.DataFrame(projection_results)
st.markdown("### Projected Age and Odometer at -30% Degradation")
st.table(projection_df.style.hide(axis='index'))
