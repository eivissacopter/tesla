...
Contains data fetching and cleaning logic.
"""
...
def clean_numeric_col(series, remove_unit=None):
    """
    Clean a pandas Series to convert to numeric.
    ...
    Returns:
        pandas Series converted to numeric
    """
    # Remove specific unit if provided
    if remove_unit:
        series = series.str.replace(remove_unit, '')
...
def fetch_data(username_filter=None):
    ...
    Args:
        username_filter: Optional username string to filter results
    
    Returns:
        Tuple of (DataFrame, battery_pack_column_name)
    """
    ...
    # Clean up the 'Odometer' column to ensure it is numeric - FIX: Use raw string
    ...
    # Clean 'Rated Range' using helper function
    df['Rated Range'] = clean_numeric_col(df['Rated Range'], remove_unit=' km')

    # Clean 'Capacity Net Now' using helper function
    ...
    # Clean 'Daily SOC Limit' and 'DC Ratio' columns using helper function
    df['Daily SOC Limit'] = clean_numeric_col(df['Daily SOC Limit'].str.replace('%', '').replace('', np.nan))
    df['DC Ratio'] = clean_numeric_col(df['DC Ratio'].str.replace('%', '').replace('', np.nan))
...
def fetch_battery_info():
    ...
    # FIX: Replace deprecated applymap with map
