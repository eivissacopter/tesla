...
class PerformanceFolder:
    ...
    def get_legend_label(self) -> str:
        ...
        """
        return (f"{self.model} {self.variant} {self.model_year} "
...
class SOHProjection:
    ...
    def get_display_text(self) -> str:
        ...
        """
        text = (f"<span style='color:orange; font-weight:bold;'>{self.battery_type}</span> "
                f"is expected to reach <span style='color:orange; font-weight:bold;'>70% SOH</span> after ")
        
        if self.years and self.years != "unknown" and self.kilometers and self.kilometers != "unknown":
            text += (f"<span style='color:orange; font-weight:bold;'>{self.years}</span> or "
                    f"<span style='color:orange; font-weight:bold;'>{self.kilometers}</span>.")
        elif self.years and self.years != "unknown":
            text += f"<span style='color:orange; font-weight:bold;'>{self.years}</span>."
        elif self.kilometers and self.kilometers != "unknown":
            text += f"<span style='color:orange; font-weight:bold;'>{self.kilometers}</span>."
        else:
            return (f"There is insufficient data to project the 70% SOH for the "
