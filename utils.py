"""
Utility functions for data fetching and cleaning in the Tesla Battery Analysis Dashboard.
"""
...
def clean_numeric_col(series, remove_unit=None):
    """
    Clean a numeric column by removing units, replacing commas with dots, and converting to numeric.
    
    Parameters:
    -----------
    series : pd.Series
        The pandas Series to clean
    remove_unit : str, optional
        The unit string to remove (e.g., ' km', ' kWh', '%')
...
def fetch_data(username_filter=None):
    """
    Fetch data from Google Sheets and clean it for analysis.
    
    Parameters:
    -----------
    username_filter : str, optional
        Filter data by username (case-insensitive)
    
    Returns:
    --------
    tuple
        (DataFrame, battery_pack_col) - The cleaned DataFrame and the name of the battery pack column
    """
    ...
    # Use raw string for regex to avoid DeprecationWarning
...
def fetch_battery_info():
    ...
    --------
    pd.DataFrame
        Battery pack information DataFrame
    """
    scope = ["https://spreadsheets.google.com/feeds", "https://www.googleapis.com/auth/drive"]
    creds_dict = {
        "type": st.secrets["gcp_service_account"]["type"],
