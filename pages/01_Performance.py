    ...
    for column in selected_columns:
        ...
        if isinstance(y_col, list):
            ...
            if 'Battery power' in y_col:
                y_values = y_values[y_values >= 40]  # Filter battery power values below 40 kW
                df_selected = df[y_col].loc[y_values.index]
                if y_values.size == 0:
                    continue  # Skip if no data after filtering
            else:
...
if plot_data:
    ...
    # Update layout to position legend below the chart
    fig.update_layout(
        showlegend=True,
    ...
    # Slider for smoothing (optional, currently not used)
    # If you decide to reintroduce smoothing in the future, you can implement it here.
